...
plot_fit<-function(m,focal_var,inter_var=NULL,RE=NULL,offset=NULL,n=20,n_core=4,boot_mer=FALSE){
  ...
  isRE<-which(names(dat)%in%RE)
  if(length(isRE)==1){
    x3<-list(unique(dat[,isRE]))
    names(x3)<-RE
  }
  if(length(isRE)>1){
    x3<-lapply(dat[,isRE],unique)
  }
  if(length(isRE)==0){
    x3<-NULL
  }
  ...
  if(length(name_f)>0){
    if(cl=="glmerMod" | cl=="lmerMod" | cl=='glmmadmb'){
  ...
  if(cl=="glmerMod" | cl=="lmerMod"){
    ...
    if(boot_mer){
      ...
      pred$LSE<-linkinv(pred$fit-se)
      pred$USE<-linkinv(pred$fit+se)
  ...
  if(cl=='glmmadmb') {
    pred$pred<-linkinv(pred$fit)
    #pred$LC<-linkinv(pred$LC)
    #pred$UC<-linkinv(pred$UC)
    #se <- sqrt(diag(mm %*% tcrossprod(vcov(m),mm)))
    #pred$LC <- linkinv(pred$fit - 1.96 * sqrt(pred$se))
    se <- sqrt(diag(mm %*% tcrossprod(vcov(m),mm)))
    pred$LSE<-linkinv(pred$fit-se)
    pred$USE<-linkinv(pred$fit+se)
    pred$LC <- linkinv(pred$fit - 1.96 * se)
    pred$UC <- linkinv(pred$fit + 1.96 * se)
    #pred$LSE<-linkinv(pred$fit-pred$se)
    #pred$USE<-linkinv(pred$fit+pred$se)
    #pred$LC <- linkinv(pred$fit) - 1.96 * pred$se*sqrt(nrow(m$frame))/4
    #pred$UC <- linkinv(pred$fit) + 1.96 * pred$se*sqrt(nrow(m$frame))/4
  }
  
  #the output
  out<-as.data.frame(cbind(new_dat[,1:(length(inter_var)+1)],pred$LC,pred$LSE,pred$pred,pred$UC,pred$USE))
  names(out)<-c(names(new_dat)[1:(length(inter_var)+1)],"LC","LSE","Pred","UC","USE")
  return(out)
}