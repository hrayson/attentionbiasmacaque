...
g <- ggplot(all_data_filt)+
 geom_point(aes(x=EmotionBiasOnscreenAmbigOther.threat, y=SelfDirectFreq, colour=Group), alpha = 1) +
 ...
 geom_ribbon(aes(x=CEmotionBiasOnscreenAmbigOther.threat,y=Pred, fill=Group,ymin=LSE,ymax=USE, colour=Group), data=group_pred, alpha=1)+
