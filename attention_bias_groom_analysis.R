...
dropbox_dir='C:/Users/Holly Rayson/Dropbox/'
# Read gaze data
gaze_data<-read.csv(file=paste0(dropbox_dir,'/joint_attention/juvenile_monkeys/pref_gaze/data/year1/all_facial_gestures_gaze_summary.csv'),header=TRUE,sep=',')

sex_file<-paste0(dropbox_dir,'/joint_attention/juvenile_monkeys/juvenile_sex_ids.csv')
...
#Load rank data
rank_file<-paste0(dropbox_dir,'/joint_attention/juvenile_monkeys/rank/rank.csv')
...
obs_data<-read.csv(file=paste0(dropbox_dir,'/joint_attention/juvenile_monkeys/all_group-ind_obs_summary.csv'),header=TRUE,sep=',')
...
obs_data$TotalGroomIDInterruptedBoutDur<-obs_data$TotalGroomIDInterruptedDur/obs_data$TotalGroomIDInterruptedFreq
obs_data$TotalGroomIDInterruptedBoutDur[is.nan(obs_data$TotalGroomIDInterruptedBoutDur)]<-0
...
for (i in 1:length(subjects)) {
  ...
  mean_val<-mean(subj_data$TotalGroomIDInterruptedDur)
  sd_val<-sd(subj_data$TotalGroomIDInterruptedDur)
  outliers<-which(abs(subj_data$TotalGroomIDInterruptedDur)>mean_val+obs_sds*sd_val)
  if (length(outliers)==0) {
    subj_data_filt<-subj_data
  } else{
    subj_data_filt<-subj_data[-outliers, ]
...
print('Group')
glmernb_group_model <- glmer.nb(TotalGroomIDInterruptedDur ~ Group*CEmotionBiasOnscreenAmbigOther.threat+Cscore_m+(1|Subject), data = all_data_filt, control = glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e6)))
...
summary_data_filt<-summaryBy(EmotionBiasOnscreenAmbigOther.threat+TotalGroomIDInterruptedDur~Subject+Group, data = all_data_filt, keep.names = TRUE)
...
g <- ggplot(all_data_filt)+
 geom_point(aes(x=EmotionBiasOnscreenAmbigOther.threat, y=TotalGroomIDInterruptedDur, colour=Group), alpha = 0.2, position='jitter') +
 geom_point(data=summary_data_filt, aes(x=EmotionBiasOnscreenAmbigOther.threat, y=TotalGroomIDInterruptedDur, colour=Group), size=3)+
...
print('Group')
glmernb_group_model <- glmer.nb(TotalGroomIDInterruptedDur ~ Group*CEmotionBiasOnscreenAmbigOther.lps+Cscore_m+(1|Subject), data = all_data_filt, control = glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e6)))
...
summary_data_filt<-summaryBy(EmotionBiasOnscreenAmbigOther.lps+TotalGroomIDInterruptedDur~Subject+Group, data = all_data_filt, keep.names = TRUE)
...
dev.new()
g <- ggplot(all_data_filt)+
 geom_point(aes(x=EmotionBiasOnscreenAmbigOther.lps, y=TotalGroomIDInterruptedDur, colour=Group), alpha = 1, position='jitter') +
 geom_point(data=summary_data_filt, aes(x=EmotionBiasOnscreenAmbigOther.lps, y=TotalGroomIDInterruptedDur, colour=Group), size=3)+
 geom_line(aes(x=CEmotionBiasOnscreenAmbigOther.lps, y=Pred, colour=Group), data=group_pred, size=1) +
 geom_ribbon(aes(x=CEmotionBiasOnscreenAmbigOther.lps,y=Pred, fill=Group,ymin=LSE,ymax=USE, colour=Group), data=group_pred, alpha=1)+
 scale_colour_brewer(palette='Set1')+theme_bw()
print(g)
...
for (i in 1:length(subjects)) {
  subj_data<-obs_data[obs_data$Subject==subjects[i],]
  mean_val<-mean(subj_data$TotalGroomIDInterruptedFreq)
  sd_val<-sd(subj_data$TotalGroomIDInterruptedFreq)
  outliers<-which(abs(subj_data$TotalGroomIDInterruptedFreq)>mean_val+obs_sds*sd_val)
  if (length(outliers)==0) {
    subj_data_filt<-subj_data
  } else{
    subj_data_filt<-subj_data[-outliers, ]
...
# Just group observation data: Mixed model with group and threat bias as fixed effects
print('Group')
glmernb_group_model <- glmer.nb(TotalGroomIDInterruptedFreq ~ Group*CEmotionBiasOnscreenAmbigOther.threat+Cscore_m+(1|Subject), data = all_data_filt, control = glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e6)))
print(glmernb_group_model)
...
summary_data_filt<-summaryBy(EmotionBiasOnscreenAmbigOther.threat+TotalGroomIDInterruptedFreq~Subject+Group, data = all_data_filt, keep.names = TRUE)
group_pred<-plot_fit(glmernb_group_model,focal_var = "CEmotionBiasOnscreenAmbigOther.threat",inter_var = c("Group"),RE = "Subject")
group_pred$CEmotionBiasOnscreenAmbigOther.threat<-(group_pred$CEmotionBiasOnscreenAmbigOther.threat*sd(wide_summary_gaze_data$EmotionBiasOnscreenAmbigOther.threat))+mean(wide_summary_gaze_data$EmotionBiasOnscreenAmbigOther.threat)

g <- ggplot(all_data_filt)+
 geom_point(aes(x=EmotionBiasOnscreenAmbigOther.threat, y=TotalGroomIDInterruptedFreq, colour=Group), alpha = 0.2, position='jitter') +
 geom_point(data=summary_data_filt, aes(x=EmotionBiasOnscreenAmbigOther.threat, y=TotalGroomIDInterruptedFreq, colour=Group), size=3)+
 geom_line(aes(x=CEmotionBiasOnscreenAmbigOther.threat, y=Pred, colour=Group), data=group_pred, size=1) +
 geom_ribbon(aes(x=CEmotionBiasOnscreenAmbigOther.threat,y=Pred, fill=Group,ymin=LSE,ymax=USE, colour=Group), data=group_pred, alpha=0.5)+
 scale_colour_brewer(palette='Set1')+theme_bw()
print(g)
...
# Just group observation data: Mixed model with group and lps bias as fixed effects
print('Group')
glmernb_group_model <- glmer.nb(TotalGroomIDInterruptedFreq ~ Group*CEmotionBiasOnscreenAmbigOther.lps+Cscore_m+(1|Subject), data = all_data_filt, control = glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e6)))
print(glmernb_group_model)
...
summary_data_filt<-summaryBy(EmotionBiasOnscreenAmbigOther.lps+TotalGroomIDInterruptedFreq~Subject+Group, data = all_data_filt, keep.names = TRUE)
group_pred<-plot_fit(glmernb_group_model,focal_var = "CEmotionBiasOnscreenAmbigOther.lps",inter_var = c("Group"),RE = "Subject")
group_pred$CEmotionBiasOnscreenAmbigOther.lps<-(group_pred$CEmotionBiasOnscreenAmbigOther.lps*sd(wide_summary_gaze_data$EmotionBiasOnscreenAmbigOther.lps))+mean(wide_summary_gaze_data$EmotionBiasOnscreenAmbigOther.lps)

g <- ggplot(all_data_filt)+
 geom_point(aes(x=EmotionBiasOnscreenAmbigOther.lps, y=TotalGroomIDInterruptedFreq, colour=Group), alpha = 1, position='jitter') +
 geom_point(data=summary_data_filt, aes(x=EmotionBiasOnscreenAmbigOther.lps, y=TotalGroomIDInterruptedFreq, colour=Group), size=3)+
 geom_line(aes(x=CEmotionBiasOnscreenAmbigOther.lps, y=Pred, colour=Group), data=group_pred, size=1) +
 geom_ribbon(aes(x=CEmotionBiasOnscreenAmbigOther.lps,y=Pred, fill=Group,ymin=LSE,ymax=USE, colour=Group), data=group_pred, alpha=1)+
 scale_colour_brewer(palette='Set1')+theme_bw()
print(g)
